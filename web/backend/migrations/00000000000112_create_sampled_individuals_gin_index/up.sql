...
CREATE FUNCTION f_concat_sampled_individuals_notes_barcode(notes text, barcode text) RETURNS text AS $$ BEGIN
    CASE
        WHEN notes IS NULL
...
CREATE INDEX sampled_individuals_notes_barcode_trgm_idx ON sampled_individuals USING gin (
    f_concat_sampled_individuals_notes_barcode(notes, barcode) gin_trgm_ops
);