    ...
    fn update(
        ...
        match (row, crud) {
            (
                ...
                if self.child_container_id == procedure_model_trackables.id {
                    foreign_keys.child_container = Some(procedure_model_trackables.clone());
                    updated = true;
                }
                if self.parent_container_id == procedure_model_trackables.id {
                    foreign_keys.parent_container = Some(procedure_model_trackables.clone());
                    updated = true;
                }
            }
            (
