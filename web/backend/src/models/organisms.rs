...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
       ...
       diesel::select(can_view_organisms(author_user_id, id))
...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
        ...
        query
            .filter(can_view_organisms(author_user_id, organisms::dsl::id))
...
let (samples0, samples1, samples2) = diesel::alias!(crate::schema::samples as samples0, crate::schema::samples as samples1, crate::schema::samples as samples2);
 if filter.map(|f| f.nameplate_id.is_some()&&f.project_id.is_some()&&f.created_by.is_some()&&f.updated_by.is_some()).unwrap_or(false) {
       unimplemented!();
 }
if let Some(nameplate_id) = filter.and_then(|f| f.nameplate_id) {
        return organisms::dsl::organisms
            .filter(organisms::dsl::nameplate_id.eq(nameplate_id))
   ...
   sample_states::dsl::sample_states.on(
       samples2.field(samples::dsl::state_id).eq(
           sample_states::dsl::id
            ...
            .filter(organisms::dsl::host_organism_id.eq(filter.and_then(|f| f.host_organism_id)))
            .filter(organisms::dsl::sample_id.eq(filter.and_then(|f| f.sample_id)))
            .filter(can_view_organisms(author_user_id, organisms::dsl::id))
            .filter(
similarity_op(nameplates::dsl::barcode, query)    .or(
...
if let Some(project_id) = filter.and_then(|f| f.project_id) {
        return organisms::dsl::organisms
            .filter(organisms::dsl::project_id.eq(project_id))
            .select(Organism::as_select())
            // This operation is defined by a first order index linking organisms.nameplate_id to nameplates.
.inner_join(
   nameplates::dsl::nameplates.on(
...
if let Some(created_by) = filter.and_then(|f| f.created_by) {
        return organisms::dsl::organisms
            .filter(organisms::dsl::created_by.eq(created_by))
            .select(Organism::as_select())
            // This operation is defined by a first order index linking organisms.nameplate_id to nameplates.
.inner_join(
   nameplates::dsl::nameplates.on(
...
if let Some(updated_by) = filter.and_then(|f| f.updated_by) {
        return organisms::dsl::organisms
            .filter(organisms::dsl::updated_by.eq(updated_by))
            .select(Organism::as_select())
            // This operation is defined by a first order index linking organisms.nameplate_id to nameplates.
.inner_join(
   nameplates::dsl::nameplates.on(
   ...
   sample_states::dsl::sample_states.on(
       samples2.field(samples::dsl::state_id).eq(
           sample_states::dsl::id
            ...
            .filter(can_view_organisms(author_user_id, organisms::dsl::id))
            .filter(
word_similarity_op(nameplates::dsl::barcode, query)    .or(
...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
       ...
       diesel::select(can_update_organisms(author_user_id, id))
            .get_result(connection).map_err(web_common::api::ApiError::from)
}
...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
        ...
        query
            .filter(can_update_organisms(author_user_id, organisms::dsl::id))
            .offset(offset.unwrap_or(0))
            .limit(limit.unwrap_or(10))
            .load::<Self>(connection).map_err(web_common::api::ApiError::from)
    }
...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
       ...
       diesel::select(can_admin_organisms(author_user_id, id))
            .get_result(connection).map_err(web_common::api::ApiError::from)
}
...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
        ...
        query
            .filter(can_admin_organisms(author_user_id, organisms::dsl::id))
            .offset(offset.unwrap_or(0))
            .limit(limit.unwrap_or(10))
            .load::<Self>(connection).map_err(web_common::api::ApiError::from)
    }
...
connection: &mut PooledConnection<ConnectionManager<diesel::prelude::PgConnection>>,
        ...
        diesel::delete(organisms::dsl::organisms
            .filter(organisms::dsl::id.eq(id))
