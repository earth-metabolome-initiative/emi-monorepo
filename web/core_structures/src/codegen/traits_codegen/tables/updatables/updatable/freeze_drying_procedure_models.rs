...
where
    ...
    >,
    crate::codegen::structs_codegen::tables::freeze_drier_models::FreezeDrierModel: web_common_traits::database::Updatable<
        C,
        UserId = i32,
    >,
{
    ...
    fn can_update(
        ...
        if !self.procedure_freeze_dried_with(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
