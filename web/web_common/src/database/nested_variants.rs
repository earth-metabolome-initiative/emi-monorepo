...
impl NestedBioOttRank {
    ...
    {
        let flat_variant = BioOttRank::get(id, connection).await?;
        match flat_variant {
...
impl NestedBioOttTaxonItem {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = BioOttTaxonItem::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedDerivedSample {
    ...
    {
        let flat_variant =
            DerivedSample::get((parent_sample_id, child_sample_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            DerivedSample::all_by_updated_at(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedDocumentFormat {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = DocumentFormat::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedLoginProvider {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = LoginProvider::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedMaterial {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Material::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedNameplateCategory {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = NameplateCategory::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedNameplate {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Nameplate::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedNotification {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Notification::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedObservationSubject {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = ObservationSubject::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedObservation {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Observation::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            Observation::all_by_updated_at(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedOrganismBioOttTaxonItem {
    ...
    {
        let flat_variant =
            OrganismBioOttTaxonItem::get((organism_id, taxon_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedOrganism {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Organism::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedOrganization {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Organization::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedPermanenceCategory {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = PermanenceCategory::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedProjectState {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = ProjectState::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedProject {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Project::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedProjectsTeamsRoleInvitation {
    ...
    {
        let flat_variant =
            ProjectsTeamsRoleInvitation::get((table_id, team_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            ProjectsTeamsRoleInvitation::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedProjectsTeamsRoleRequest {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = ProjectsTeamsRoleRequest::get((table_id, team_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            ProjectsTeamsRoleRequest::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedProjectsTeamsRole {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = ProjectsTeamsRole::get((table_id, team_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedProjectsUsersRoleInvitation {
    ...
    {
        let flat_variant =
            ProjectsUsersRoleInvitation::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            ProjectsUsersRoleInvitation::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedProjectsUsersRoleRequest {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = ProjectsUsersRoleRequest::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            ProjectsUsersRoleRequest::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedProjectsUsersRole {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = ProjectsUsersRole::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedRole {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Role::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedSampleBioOttTaxonItem {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = SampleBioOttTaxonItem::get((sample_id, taxon_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedSampleContainerCategory {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = SampleContainerCategory::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedSampleContainer {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = SampleContainer::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            SampleContainer::all_by_updated_at(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedSampleState {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = SampleState::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedSample {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Sample::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedSpectra {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Spectra::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedSpectraCollection {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = SpectraCollection::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            SpectraCollection::all_by_updated_at(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedTeamState {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = TeamState::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedTeam {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Team::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedTeamsTeamsRoleInvitation {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = TeamsTeamsRoleInvitation::get((table_id, team_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            TeamsTeamsRoleInvitation::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedTeamsUsersRoleInvitation {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = TeamsUsersRoleInvitation::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            TeamsUsersRoleInvitation::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedTeamsUsersRoleRequest {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = TeamsUsersRoleRequest::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedTeamsUsersRole {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = TeamsUsersRole::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedUnit {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = Unit::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedUserEmail {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = UserEmail::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedUser {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = User::get(id, connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedUsersUsersRoleInvitation {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = UsersUsersRoleInvitation::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
    ...
    {
        let flat_variants =
            UsersUsersRoleInvitation::all(filter, limit, offset, connection).await?;
        let mut nested_structs = Vec::with_capacity(flat_variants.len());
        for flat_variant in flat_variants {
            nested_structs.push(Self::from_flat(flat_variant, connection).await?);
        }
...
impl NestedUsersUsersRoleRequest {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = UsersUsersRoleRequest::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
...
impl NestedUsersUsersRole {
    ...
    where
        C: gluesql::core::store::GStore + gluesql::core::store::GStoreMut,
    {
        let flat_variant = UsersUsersRole::get((table_id, user_id), connection).await?;
        match flat_variant {
            Some(flat_variant) => Ok(Some(Self::from_flat(flat_variant, connection).await?)),
            None => Ok(None),
        }
