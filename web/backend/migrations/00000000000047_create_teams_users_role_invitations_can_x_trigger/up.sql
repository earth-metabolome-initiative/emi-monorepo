-- The function `can_update_teams_users_role_invitations` takes a user ID (INTEGER) and the primary keys
...
CREATE FUNCTION can_update_teams_users_role_invitations(author_user_id INTEGER, table_id INTEGER, user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- We retrieve the value of the parent column from the row, as identified by the provided primary key(s).
    SELECT table_id, user_id, created_by, 1 INTO table_id, user_id, created_by, canary FROM teams_users_role_invitations WHERE teams_users_role_invitations.table_id = table_id AND teams_users_role_invitations.user_id = user_id;
...
-- We check whether the user is the created_by of the row.
    IF author_user_id = created_by THEN
        RETURN TRUE;
    END IF;
        IF can_update_teams(author_user_id, table_id) THEN
            RETURN TRUE;
        END IF;
...
-- checks if the row was created by the user or if the user is found in either the teams_users_role_invitations_users_roles table or
-- the teams_users_role_invitations_teams_users table with an appropriate role id.
CREATE FUNCTION can_admin_teams_users_role_invitations(author_user_id INTEGER, table_id INTEGER, user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    table_id INTEGER;
...
-- We check whether the user is the created_by of the row.
    IF author_user_id = created_by THEN
        RETURN TRUE;
    END IF;
        IF can_admin_teams(author_user_id, table_id) THEN
            RETURN TRUE;
        END IF;
...
-- The function `can_view_teams_users_role_invitations` takes a user ID (INTEGER) and the primary keys
-- and returns a BOOLEAN indicating whether the user can {operation} the row. Since this table's editability
-- may depend on the parent column, this function retrieves the value of the parent column from the row
-- and calls the parent column's can_view function if the parent column is not NULL. Otherwise, the function
-- checks if the row was created by the user or if the user is found in either the teams_users_role_invitations_users_roles table or
-- the teams_users_role_invitations_teams_users table with an appropriate role id.
CREATE FUNCTION can_view_teams_users_role_invitations(author_user_id INTEGER, table_id INTEGER, user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    table_id INTEGER;
...
-- We check whether the user is the created_by of the row.
    IF author_user_id = created_by THEN
        RETURN TRUE;
    END IF;
        IF can_view_teams(author_user_id, table_id) THEN
            RETURN TRUE;
        END IF;
