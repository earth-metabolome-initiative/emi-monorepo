...
impl Column {
    pub(super) fn builder_trait_where_constraints(
        ...
        let column_acronym = self.acronym_ident()?;
        ...
        if self.is_foreign_primary_key(conn)? || self.is_part_of_primary_key(conn)? {
            let argument_type = self.to_non_nullable().rust_data_type(conn)?;
            if self.is_nullable() {
                Ok(Some(quote! {
                    where
            ...
            let argument_type = self.rust_data_type(conn)?;
