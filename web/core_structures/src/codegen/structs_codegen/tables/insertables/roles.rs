...
impl core::str::FromStr for InsertableRoleAttributes {
...
/// tables.
pub trait RoleBuildable: std::marker::Sized {
...
impl RoleBuildable for Option<i16> {
    type Attributes =
...
impl RoleBuildable for InsertableRoleBuilder {
    type Attributes =
        crate::codegen::structs_codegen::tables::insertables::InsertableRoleAttributes;
    /// Sets the value of the `public.roles.name` column.
    fn name<'N, N>(
        ...
        validation_errors::SingleFieldError: From<<&'N N as TryInto<String>>::Error>,
    {
        let name = name.try_into().map_err(|err| {
            validation_errors::SingleFieldError::from(err)
                .rename_field(InsertableRoleAttributes::Name)
        })?;
        self.name = Some(name);
        Ok(self)
    }
        ...
        validation_errors::SingleFieldError: From<<&'D D as TryInto<String>>::Error>,
    {
        let description = description.try_into().map_err(|err| {
            validation_errors::SingleFieldError::from(err)
                .rename_field(InsertableRoleAttributes::Description)
        })?;
        ...
        validation_errors::SingleFieldError: From<<&'I I as TryInto<String>>::Error>,
    {
        let icon = icon.try_into().map_err(|err| {
            validation_errors::SingleFieldError::from(err)
                .rename_field(InsertableRoleAttributes::Icon)
        })?;
        self.icon = Some(icon);
        Ok(self)
    }
    ...
    fn color(
        ...
        let color_id = color_id.try_into().map_err(|err| {
            validation_errors::SingleFieldError::from(err)
                .rename_field(InsertableRoleAttributes::ColorId)
        })?;
