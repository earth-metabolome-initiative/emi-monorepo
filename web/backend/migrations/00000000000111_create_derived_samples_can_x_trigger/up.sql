...
-- We check whether the user is the updated_by of the row.
    IF author_user_id = this_updated_by THEN
        RETURN TRUE;
    END IF;
        IF NOT can_admin_samples(author_user_id, this_parent_sample_id) THEN
            RETURN FALSE;
        END IF;
        IF NOT can_admin_samples(author_user_id, this_child_sample_id) THEN
            RETURN FALSE;
        END IF;
    RETURN FALSE;
END;
...
BEGIN
    IF TG_OP = 'UPDATE' THEN
        IF NOT can_update_derived_samples(NEW.updated_by, ( NEW.parent_sample_id, NEW.child_sample_id )) THEN
            RAISE EXCEPTION 'unauthorized_update' USING DETAIL = 'Unauthorized to update this row of the `derived_samples` table.';
        END IF;
    END IF;
