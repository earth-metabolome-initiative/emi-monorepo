...
impl diesel::Identifiable for StepModelReagent {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
}
impl StepModelReagent {
    ...
    pub async fn from_id(
        conn: &mut diesel_async::AsyncPgConnection,
        id: &crate::codegen::structs_codegen::tables::step_models::StepModel,
    ) -> Result<Self, diesel::result::Error> {
        use diesel::{ExpressionMethods, QueryDsl, associations::HasTable};
        use diesel_async::RunQueryDsl;
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::step_model_reagents::step_model_reagents::dsl::id
                    .eq(id.id),
            )
            .first::<Self>(conn)
            .await
    ...
    pub async fn from_reagent_id(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::step_model_reagents::step_model_reagents::dsl::reagent_id
                    .eq(reagent_id.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_created_by(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::step_model_reagents::step_model_reagents::dsl::created_by
                    .eq(created_by.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_updated_by(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::step_model_reagents::step_model_reagents::dsl::updated_by
                    .eq(updated_by.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_created_at(
        ...
        use crate::codegen::diesel_codegen::tables::step_model_reagents::step_model_reagents;
        Self::table()
            .filter(step_model_reagents::created_at.eq(created_at))
            .order_by(step_model_reagents::id.asc())
            .load::<Self>(conn)
            .await
    }
    #[cfg(feature = "postgres")]
    pub async fn from_updated_at(
        ...
        use crate::codegen::diesel_codegen::tables::step_model_reagents::step_model_reagents;
        Self::table()
            .filter(step_model_reagents::updated_at.eq(updated_at))
            .order_by(step_model_reagents::id.asc())
            .load::<Self>(conn)
            .await
    }
