    ...
    >,
{
    ...
    fn can_update(
        ...
        if !self.stratified_source(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.supernatant_destination(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.transferred_with(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        Ok(true)
    }
