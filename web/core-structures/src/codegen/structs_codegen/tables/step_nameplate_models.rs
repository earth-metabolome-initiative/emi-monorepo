...
impl StepNameplateModel {
    ...
    pub async fn from_created_at(
        ...
        use crate::codegen::diesel_codegen::tables::step_nameplate_models::step_nameplate_models;
        Self::table()
            .filter(step_nameplate_models::created_at.eq(created_at))
            .order_by(step_nameplate_models::id.asc())
            .load::<Self>(conn)
            .await
    }
}
