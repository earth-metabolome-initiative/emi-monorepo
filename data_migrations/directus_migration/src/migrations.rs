...
mod utils;
use core_structures::{tables::insertables::InsertableProcedureModelBuilder, ProcedureModel};
...
fn process_sample(
	...
	if let Some(ExpectedVariant1(procedure_builder)) = procedure_builders.next() {
	...
	if let Some(ExpectedVariant2(procedure_builder)) = procedure_builders.next() {
		assert_eq!(procedure_builder.name, "Second step of the DBGI plan");
		procedure_builder
			.insert_first_trackable(trackable)
			.insert(portal_conn)?;
	} else {
	...
	if let Some(ExpectedVariant3(procedure_builder)) = procedure_builders.next() {
		assert_eq!(procedure_builder.name, "Third step of the DBGI plan");
		procedure_builder
			.insert_first_trackable(trackable)
			.insert(portal_conn)?;
	} else {
...
pub fn directus_migration(
	...
	let procedure_builders: impl Iterator<Item = Enum<InsertableProcedureBuilder>> = dbgi_pm.subprocedures(portal_conn);
