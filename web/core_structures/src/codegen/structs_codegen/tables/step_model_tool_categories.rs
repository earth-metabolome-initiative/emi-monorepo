...
pub struct StepModelToolCategory {
    ...
    pub procedure_model_tool_category_id: i32,
    pub created_by: i32,
    pub created_at: ::rosetta_timestamp::TimestampUTC,
    pub updated_by: i32,
    pub updated_at: ::rosetta_timestamp::TimestampUTC,
}
impl diesel::Identifiable for StepModelToolCategory {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
...
impl StepModelToolCategory {
    ...
    #[cfg(feature = "postgres")]
    pub async fn from_procedure_model_tool_category_id(
        conn: &mut diesel_async::AsyncPgConnection,
        procedure_model_tool_category_id: &crate::codegen::structs_codegen::tables::procedure_model_tool_categories::ProcedureModelToolCategory,
    ) -> Result<Vec<Self>, diesel::result::Error> {
        use diesel::{ExpressionMethods, QueryDsl, associations::HasTable};
        use diesel_async::RunQueryDsl;
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::step_model_tool_categories::step_model_tool_categories::dsl::procedure_model_tool_category_id
                    .eq(procedure_model_tool_category_id.id),
            )
            .load::<Self>(conn)
            .await
    }
    ...
    pub async fn from_created_at(
        ...
        use crate::codegen::diesel_codegen::tables::step_model_tool_categories::step_model_tool_categories;
        Self::table()
            .filter(step_model_tool_categories::created_at.eq(created_at))
            .order_by(step_model_tool_categories::id.asc())
            .load::<Self>(conn)
            .await
    }
    #[cfg(feature = "postgres")]
    pub async fn from_updated_at(
        ...
        use crate::codegen::diesel_codegen::tables::step_model_tool_categories::step_model_tool_categories;
        Self::table()
            .filter(step_model_tool_categories::updated_at.eq(updated_at))
            .order_by(step_model_tool_categories::id.asc())
            .load::<Self>(conn)
            .await
    }
