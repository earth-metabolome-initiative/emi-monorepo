impl From<super::Rows> for Vec<crate::codegen::tables::row::Row> {
    fn from(rows: super::Rows) -> Self {
        match rows {
            ...
            super::Rows::AliquotingInstrumentModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::AliquotingInstrumentModel)
                .collect::<Vec<_>>(),
            super::Rows::AliquotingProcedureModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::AliquotingProcedureModel)
                .collect::<Vec<_>>(),
            super::Rows::BallMillProcedureModel(rows) => rows
                .into_iter()
            ...
            super::Rows::CentrifugeProcedureModel(rows) => rows
                .into_iter()
            ...
            super::Rows::DisposalProcedureModel(rows) => rows
                .into_iter()
            ...
            super::Rows::EmailProvider(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::EmailProvider)
                .collect::<Vec<_>>(),
            super::Rows::FractioningProcedureModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::FractioningProcedureModel)
                .collect::<Vec<_>>(),
            super::Rows::FreezeDryingProcedureModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::FreezeDryingProcedureModel)
                .collect::<Vec<_>>(),
            super::Rows::FreezingProcedureModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::FreezingProcedureModel)
                .collect::<Vec<_>>(),
            super::Rows::InstrumentModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::InstrumentModel)
                .collect::<Vec<_>>(),
            super::Rows::InstrumentState(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::InstrumentState)
                .collect::<Vec<_>>(),
            super::Rows::Instrument(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::Instrument)
                .collect::<Vec<_>>(),
            super::Rows::LoginProvider(rows) => rows
                .into_iter()
            ...
            super::Rows::SpectraCollection(rows) => rows
                .into_iter()
            ...
            super::Rows::TeamMember(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::TeamMember)
                .collect::<Vec<_>>(),
            super::Rows::TeamProject(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::TeamProject)
                .collect::<Vec<_>>(),
            super::Rows::TeamState(rows) => rows
                .into_iter()
            ...
            super::Rows::TemporaryUser(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::TemporaryUser)
                .collect::<Vec<_>>(),
            super::Rows::TrackableLocation(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::TrackableLocation)
                .collect::<Vec<_>>(),
            super::Rows::Trackable(rows) => rows
                .into_iter()
            ...
            super::Rows::UserEmail(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::UserEmail)
                .collect::<Vec<_>>(),
            super::Rows::UserOrganization(rows) => rows
                .into_iter()
            ...
            super::Rows::VolumetricProcessable(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::VolumetricProcessable)
                .collect::<Vec<_>>(),
            super::Rows::WeighingInstrumentModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::WeighingInstrumentModel)
                .collect::<Vec<_>>(),
            super::Rows::WeighingProcedureModel(rows) => rows
                .into_iter()
                .map(crate::codegen::tables::row::Row::WeighingProcedureModel)
                .collect::<Vec<_>>(),
            super::Rows::WeighingProcedure(rows) => rows
                .into_iter()
