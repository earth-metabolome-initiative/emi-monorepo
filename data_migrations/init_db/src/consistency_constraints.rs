...
pub(crate) fn execute_consistency_constraint_checks(
    ...
    for schema in &["public", PROCEDURES_SCHEMA, PROCEDURE_TEMPLATES_SCHEMA] {
        let mut sub_time_tracker =
            TimeTracker::new(&format!("Check constraints in schema '{schema}'"));
    ...
    ProcedurePrimaryKeyConstraint.check_all(database_name, PROCEDURES_SCHEMA, conn)?;
    ProcedureTemplatePrimaryKeyConstraint.check_all(
        database_name,
        PROCEDURE_TEMPLATES_SCHEMA,
        conn,
    )?;
    ProcedureToProcedureTemplateForeignKeyConstraint.check_all(
        database_name,
        PROCEDURES_SCHEMA,
        conn,
    )?;
    UnusedForeignProcedureTemplateConstraint.check_all(
        database_name,
        PROCEDURE_TEMPLATES_SCHEMA,
        conn,
    )?;
    ProcedureAssetsForeignKeysConstraint.check_all(database_name, PROCEDURES_SCHEMA, conn)?;
    time_tracker.add_completed_task(task);

    let task = Task::new("Procedure and procedure template asset constraints");
    for schema in &[PROCEDURES_SCHEMA, PROCEDURE_TEMPLATES_SCHEMA] {
