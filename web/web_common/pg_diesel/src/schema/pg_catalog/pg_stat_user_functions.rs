//! Submodule for the `pg_catalog.pg_stat_user_functions` view schema.

diesel::table! {
    /// `pg_catalog.pg_stat_user_functions` — view showing statistics for user-defined functions.
    /// Each row represents one tracked function showing statistics about calls to that function.
