...
if let Some(table_id) = filter.and_then(|f| f.table_id) {
        return projects_users_role_invitations::dsl::projects_users_role_invitations
            .filter(projects_users_role_invitations::dsl::table_id.eq(table_id))
...
.inner_join(
   projects::dsl::projects.on(
       projects_users_role_invitations::dsl::table_id.eq(
           projects::dsl::id
        )
    )
)
...
.inner_join(
   roles::dsl::roles.on(
       projects_users_role_invitations::dsl::role_id.eq(
           roles::dsl::id
        )
    )
)
...
if let Some(role_id) = filter.and_then(|f| f.role_id) {
        return projects_users_role_invitations::dsl::projects_users_role_invitations
            .filter(projects_users_role_invitations::dsl::role_id.eq(role_id))
            .select(ProjectsUsersRoleInvitation::as_select())
            // This operation is defined by a first order index linking projects_users_role_invitations.table_id to projects.
.inner_join(
   projects::dsl::projects.on(
...
if let Some(created_by) = filter.and_then(|f| f.created_by) {
        return projects_users_role_invitations::dsl::projects_users_role_invitations
            .filter(projects_users_role_invitations::dsl::created_by.eq(created_by))
            .select(ProjectsUsersRoleInvitation::as_select())
            // This operation is defined by a first order index linking projects_users_role_invitations.table_id to projects.
.inner_join(
   projects::dsl::projects.on(
