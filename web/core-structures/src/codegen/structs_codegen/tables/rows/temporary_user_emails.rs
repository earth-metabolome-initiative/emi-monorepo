impl From<crate::codegen::structs_codegen::tables::temporary_user_emails::TemporaryUserEmail>
    for super::Rows
{
    fn from(
        value: crate::codegen::structs_codegen::tables::temporary_user_emails::TemporaryUserEmail,
    ) -> Self {
        Self::from(vec![value])
    }
}
impl From<Vec<crate::codegen::structs_codegen::tables::temporary_user_emails::TemporaryUserEmail>>
    for super::Rows
{
    fn from(
        value: Vec<
            crate::codegen::structs_codegen::tables::temporary_user_emails::TemporaryUserEmail,
        >,
    ) -> Self {
        super::Rows::TemporaryUserEmail(value)
    }
}
impl TryFrom<super::Rows>
    for Vec<crate::codegen::structs_codegen::tables::temporary_user_emails::TemporaryUserEmail>
{
    type Error = std::convert::Infallible;
    fn try_from(value: super::Rows) -> Result<Self, Self::Error> {
        match value {
            super::Rows::TemporaryUserEmail(v) => Ok(v),
            value => unreachable!("Unexpected variant in Rows enum: {value:?}"),
        }
    }
}
