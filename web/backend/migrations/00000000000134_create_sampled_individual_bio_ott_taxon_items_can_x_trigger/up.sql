...
CREATE FUNCTION can_update_sampled_individual_bio_ott_taxon_items(author_user_id INTEGER, sampled_individual_id UUID, taxon_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- We check whether the user is the created_by of the row.
    IF author_user_id = created_by THEN
        RETURN TRUE;
    END IF;
        IF NOT can_update_sampled_individuals(author_user_id, sampled_individual_id) THEN
            RETURN FALSE;
        END IF;
    RETURN TRUE;
END;
...
-- checks if the row was created by the user or if the user is found in either the sampled_individual_bio_ott_taxon_items_users_roles table or
-- the sampled_individual_bio_ott_taxon_items_teams_users table with an appropriate role id.
CREATE FUNCTION can_admin_sampled_individual_bio_ott_taxon_items(author_user_id INTEGER, sampled_individual_id UUID, taxon_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    sampled_individual_id UUID;
...
-- We check whether the user is the created_by of the row.
    IF author_user_id = created_by THEN
        RETURN TRUE;
    END IF;
        IF NOT can_admin_sampled_individuals(author_user_id, sampled_individual_id) THEN
            RETURN FALSE;
        END IF;
    RETURN TRUE;
END;
...
-- checks if the row was created by the user or if the user is found in either the sampled_individual_bio_ott_taxon_items_users_roles table or
-- the sampled_individual_bio_ott_taxon_items_teams_users table with an appropriate role id.
CREATE FUNCTION can_view_sampled_individual_bio_ott_taxon_items(author_user_id INTEGER, sampled_individual_id UUID, taxon_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    sampled_individual_id UUID;
