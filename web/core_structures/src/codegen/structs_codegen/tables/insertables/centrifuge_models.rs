    ...
    for InsertableCentrifugeModelAttributes
{
    fn from(
        ...
        Self::Extension(InsertableCentrifugeModelExtensionAttributes::Trackable(trackables))
    }
}
...
for InsertableCentrifugeModelBuilder<Trackable>
where
        ...
        self.id = self
            ...
            .map_err(|err| {
                err.into_field_name(|attribute| InsertableCentrifugeModelAttributes::Extension(
                    InsertableCentrifugeModelExtensionAttributes::Trackable(attribute),
                ))
            })?;
        Ok(self)
    }
    ...
    >
{
    ...
    /// columns from table `centrifuge_models`.
