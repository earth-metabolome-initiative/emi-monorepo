...
pub struct DirectusPanelForeignKeys {
    pub dashboard: Option<
        std::rc::Rc<
...
{
    type ForeignKeys = DirectusPanelForeignKeys;
        ...
        C: web_common_traits::crud::Connector<Row = Self::Row>,
    {
        connector.send(web_common_traits::crud::CrudPrimaryKeyOperation::Read(
            crate::codegen::tables::table_primary_keys::TablePrimaryKey::DirectusDashboard(
                self.dashboard,
            ),
        ));
    ...
    fn foreign_keys_loaded(&self, foreign_keys: &Self::ForeignKeys) -> bool {
        foreign_keys.dashboard.is_some()
    ...
    fn update(
        ...
        match (row, crud) {
            (
                crate::codegen::tables::row::Row::DirectusDashboard(directus_dashboards),
                web_common_traits::crud::CRUD::Read
                | web_common_traits::crud::CRUD::Create
                | web_common_traits::crud::CRUD::Update,
            ) => {
                if directus_dashboards.id == self.dashboard {
                    foreign_keys.dashboard = Some(directus_dashboards);
                    updated = true;
                }
            }
            (
...
impl web_common_traits::prelude::ForeignKeys for DirectusPanelForeignKeys {}
