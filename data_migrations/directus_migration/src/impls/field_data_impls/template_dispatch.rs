...
impl FieldDatumWrapper {
    pub(crate) fn directus_template_dispatch(&self) -> DirectusTemplate {
        match self.as_ref().qfield_project.as_str().trim() {
            ...
            "artemisia_absinthium" | "sbl_20004" | "emi" | "chemical_ecology" | "champex_gradient" | "medicin_alps" | "bioblitz_vinesch" => {
    ...
    pub(crate) fn procedure_dispatch(
        ...
        match self.directus_template_dispatch() {
            ...
            DirectusTemplate::EMIPartOfOrganism => self.emi_part_of_organism_wizard_migration(user, conn),
            DirectusTemplate::EMIWholeOrganism => self.emi_whole_organism_wizard_migration(user, conn),
