...
pub fn create_observation_form(props: &CreateObservationFormProp) -> Html {
   ...
   if let Some(organism_id) = props.organism_id {
         named_requests.push(ComponentMessage::get_named::<&str, Organism>("organism", organism_id.into()));
    }
   if let Some(sample_id) = props.sample_id {
    ...
    let set_parent_observation = builder_dispatch.apply_callback(|parent_observation: Option<NestedObservation>| ObservationActions::SetParentObservation(parent_observation));
    let set_project = builder_dispatch.apply_callback(|project: Option<NestedProject>| ObservationActions::SetProject(project));
    let set_organism = builder_dispatch.apply_callback(|organism: Option<NestedOrganism>| ObservationActions::SetOrganism(organism));
    let set_sample = builder_dispatch.apply_callback(|sample: Option<NestedSample>| ObservationActions::SetSample(sample));
    html! {
...
impl FromOperation for OrganismBioOttTaxonItemActions {
    fn from_operation<S: AsRef<str>>(operation: S, row: Vec<u8>) -> Self {
        match operation.as_ref() {
            "organism" => OrganismBioOttTaxonItemActions::SetOrganism(Some(bincode::deserialize(&row).unwrap())),
            "taxon" => OrganismBioOttTaxonItemActions::SetTaxon(Some(bincode::deserialize(&row).unwrap())),
            operation_name => unreachable!("The operation name '{}' is not supported.", operation_name),
        }
    }
}
...
impl Reducer<OrganismBioOttTaxonItemBuilder> for OrganismBioOttTaxonItemActions {
    fn apply(self, mut state: std::rc::Rc<OrganismBioOttTaxonItemBuilder>) -> std::rc::Rc<OrganismBioOttTaxonItemBuilder> {
        let state_mut = Rc::make_mut(&mut state);
        match self {
...
!self.errors_organism.is_empty() || !self.errors_taxon.is_empty()
    ...
    fn update(dispatcher: &Dispatch<Self>, richest_variant: Self::RichVariant) -> Vec<ComponentMessage> {
        dispatcher.apply(OrganismBioOttTaxonItemActions::SetOrganism(Some(richest_variant.organism)));
        dispatcher.apply(OrganismBioOttTaxonItemActions::SetTaxon(Some(richest_variant.taxon)));
        vec![]
    }
...
impl From<OrganismBioOttTaxonItemBuilder> for NewOrganismBioOttTaxonItem {
    fn from(builder: OrganismBioOttTaxonItemBuilder) -> Self {
        Self {
            organism_id: builder.organism.as_ref().map(|organism| organism.inner.id).unwrap(),
...
pub fn create_organism_bio_ott_taxon_item_form(props: &CreateOrganismBioOttTaxonItemFormProp) -> Html {
     let mut named_requests: Vec<ComponentMessage> = Vec::new();
   ...
   if let Some(taxon_id) = props.taxon_id {
    ...
    let set_organism = builder_dispatch.apply_callback(|organism: Option<NestedOrganism>| OrganismBioOttTaxonItemActions::SetOrganism(organism));
    let set_taxon = builder_dispatch.apply_callback(|taxon: Option<NestedBioOttTaxonItem>| OrganismBioOttTaxonItemActions::SetTaxon(taxon));
    html! {
...
impl FromOperation for OrganismActions {
    fn from_operation<S: AsRef<str>>(operation: S, row: Vec<u8>) -> Self {
        match operation.as_ref() {
            "host_organism" => OrganismActions::SetHostOrganism(Some(bincode::deserialize(&row).unwrap())),
        ...
        if picture.is_none() {
            ...
            OrganismActions::SetHostOrganism(host_organism) => 'host_organism: {
                ...
                match host_organism.as_ref() {
                    Some(host_organism) => {
                            if state_mut.id.map_or(false, |id| id == host_organism.inner.id)
                        {
                            state_mut.errors_host_organism.push(ApiError::BadRequest(vec![
                                "The Host organism field must be distinct from the current value.".to_string()
                             ]));
                            break 'host_organism;
                        }
                    }
...
impl FormBuilder for OrganismBuilder {
    ...
    fn has_errors(&self) -> bool {
!self.errors_notes.is_empty() || !self.errors_picture.is_empty() || !self.errors_host_organism.is_empty() || !self.errors_sample.is_empty() || !self.errors_nameplate.is_empty() || !self.errors_project.is_empty()
    }

    fn update(dispatcher: &Dispatch<Self>, richest_variant: Self::RichVariant) -> Vec<ComponentMessage> {
        dispatcher.reduce_mut(|state| {state.id = Some(richest_variant.inner.id);});
    dispatcher.apply(OrganismActions::SetNotes(richest_variant.inner.notes.map(|notes| notes.to_string())));
        dispatcher.apply(OrganismActions::SetPicture(Some(richest_variant.inner.picture)));        dispatcher.apply(OrganismActions::SetSample(richest_variant.sample));
        dispatcher.apply(OrganismActions::SetNameplate(Some(richest_variant.nameplate)));
    ...
    named_requests.push(ComponentMessage::get_named::<&str, Organism>("host_organism", host_organism_id.into()));
 } else {
...
impl From<OrganismBuilder> for NewOrganism {
    fn from(builder: OrganismBuilder) -> Self {
        Self {
            id: builder.id.unwrap_or_else(Uuid::new_v4),
            host_organism_id: builder.host_organism.map(|host_organism| host_organism.inner.id),
            sample_id: builder.sample.map(|sample| sample.inner.id),
            notes: builder.notes,
...
pub fn create_organism_form(props: &CreateOrganismFormProp) -> Html {
   ...
   if let Some(nameplate_id) = props.nameplate_id {
         named_requests.push(ComponentMessage::get_named::<&str, Nameplate>("nameplate", nameplate_id.into()));
    }
   if let Some(project_id) = props.project_id {
         named_requests.push(ComponentMessage::get_named::<&str, Project>("project", project_id.into()));
    }
    let set_notes = builder_dispatch.apply_callback(|notes: Option<String>| OrganismActions::SetNotes(notes));
    let set_picture = builder_dispatch.apply_callback(|picture: Option<Image>| OrganismActions::SetPicture(picture.map(|picture| picture.into())));
    let set_host_organism = builder_dispatch.apply_callback(|host_organism: Option<NestedOrganism>| OrganismActions::SetHostOrganism(host_organism));
    let set_sample = builder_dispatch.apply_callback(|sample: Option<NestedSample>| OrganismActions::SetSample(sample));
    let set_nameplate = builder_dispatch.apply_callback(|nameplate: Option<NestedNameplate>| OrganismActions::SetNameplate(nameplate));
    let set_project = builder_dispatch.apply_callback(|project: Option<NestedProject>| OrganismActions::SetProject(project));
    html! {
...
pub fn create_sample_container_form(props: &CreateSampleContainerFormProp) -> Html {
   ...
   if let Some(project_id) = props.project_id {
    ...
    named_requests.push(ComponentMessage::get_named::<&str, SampleContainerCategory>("category", props.category_id.into()));
...
pub fn create_sample_form(props: &CreateSampleFormProp) -> Html {
   ...
   if let Some(sampled_by) = props.sampled_by {
    ...
    named_requests.push(ComponentMessage::get_named::<&str, SampleState>("state", props.state_id.into()));
...
pub fn create_spectra_form(props: &CreateSpectraFormProp) -> Html {
   ...
   if let Some(spectra_collection_id) = props.spectra_collection_id {
         named_requests.push(ComponentMessage::get_named::<&str, SpectraCollection>("spectra_collection", spectra_collection_id.into()));
    }
