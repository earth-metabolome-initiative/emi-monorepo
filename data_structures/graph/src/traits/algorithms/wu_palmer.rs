//! Implementation for the 'Wu Palmer' trait based on the algorithm implementation

use core::f64;

use algebra::prelude::{Kahn, KahnError};
use functional_properties::prelude::ScalarSimilarity;
use crate::traits::algorithms::root_nodes::RootNodes;
use crate::traits::{Edges, MonoplexMonopartiteGraph};
    ...
    G: MonoplexMonopartiteGraph,
{
    ...
    fn similarity(&self, left: &G::NodeId, right: &G::NodeId) -> Self::Similarity {
        ...
        for root_node in &self.root_nodes {
            let (Some(n1),Some(n2),Some(n3)) = wu_palmer_depth(
                self.graph,
                0,
                *root_node,
                *left,
...
fn wu_palmer_depth<G>(
    ...
    left: G::NodeId,
    right: G::NodeId,
) -> (Option<usize>,Option<usize>,Option<usize>) where G: MonoplexMonopartiteGraph + ?Sized {
    let (mut n1,mut n2,mut n3) = (None, None, Some(depth));
    ...
    for successor in graph.successors(current_node) {
        match wu_palmer_depth(graph, depth+1, successor, left, right) {
            ...
            (rec_n1, rec_n2, Some(rec_n3)) => {
                n3 = Some(if let Some(n3) = n3 {
                    if n3 < rec_n3 {
                        n1 = rec_n1;
                        n2 = rec_n2;
                        rec_n3
            ...
            (Some(_rec_n1), Some(_rec_n2), None) => unreachable!()
        }
    }
    (n1,n2,n3)
}
