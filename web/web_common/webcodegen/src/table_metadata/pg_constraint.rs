//! Submodule providing a struct [`PgConstraint`] representing the `pg_constraint` table.
...
pub struct PgConstraint {
    ...
    /// The type of the constraint: c = check constraint, f = foreign key constraint, n = not-null constraint (domains only), p = primary key constraint, u = unique constraint, t = constraint trigger, x = exclusion constraint
    pub contype: String,
...
impl PgConstraint {
    /// Returns the vector of [`PgProc`] functions that are used in the constraint.
    ///
    /// # Arguments
    ///
