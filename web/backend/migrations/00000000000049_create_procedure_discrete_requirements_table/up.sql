...
-- table, so that when a procedure discrete requirement is deleted, the corresponding editable is also deleted.
...
-- record in the editables table when a procedure discrete requirement is deleted.
...
DELETE
    ON procedure_discrete_requirements FOR EACH ROW EXECUTE FUNCTION delete_editables();
