...
let (samples0, samples1, samples2) = diesel::alias!(crate::schema::samples as samples0, crate::schema::samples as samples1, crate::schema::samples as samples2);
 if filter.map(|f| f.sample_id.is_some()&&f.created_by.is_some()&&f.updated_by.is_some()).unwrap_or(false) {
       unimplemented!();
 }
if let Some(sample_id) = filter.and_then(|f| f.sample_id) {
   ...
   sample_states::dsl::sample_states.on(
       samples2.field(samples::dsl::state_id).eq(
           sample_states::dsl::id
            ...
            .filter(can_view_spectra_collections(author_user_id, spectra_collections::dsl::id))
            .filter(
similarity_op(sample_containers::dsl::barcode, query)    .or(
   ...
   sample_states::dsl::sample_states.on(
       samples2.field(samples::dsl::state_id).eq(
           sample_states::dsl::id
            ...
            .filter(can_view_spectra_collections(author_user_id, spectra_collections::dsl::id))
            .filter(
word_similarity_op(sample_containers::dsl::barcode, query)    .or(
   ...
   sample_states::dsl::sample_states.on(
       samples2.field(samples::dsl::state_id).eq(
           sample_states::dsl::id
            ...
            .filter(can_view_spectra_collections(author_user_id, spectra_collections::dsl::id))
            .filter(
strict_word_similarity_op(sample_containers::dsl::barcode, query)    .or(
