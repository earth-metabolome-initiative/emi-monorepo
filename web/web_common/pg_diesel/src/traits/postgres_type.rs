//! Submodule providing the [`PostgresType`] trait used throughout the `pg_diesel` crate
//! which associates a postgres string type to DB objects.
...
pub trait PostgresType {
    /// Returns the name of the postgres type associated to the current DB object.
    ///
    /// # Arguments
    ///
