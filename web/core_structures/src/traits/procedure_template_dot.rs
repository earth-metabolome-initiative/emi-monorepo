...
//! Submodule providing a `ProcedureTemplateDot` trait for creating a DOT
//! representation of a procedure template.
...
pub trait ProcedureTemplateDot: ExtensionTable<ProcedureTemplate>
where
    for<'a> &'a Self: diesel::Identifiable<Id = &'a i32>,
{
    /// Returns the name of the procedure template.
    fn name(&self, conn: &mut PgConnection) -> Result<String, diesel::result::Error> {
        use diesel::Identifiable;

        if let Some(model) = ProcedureTemplate::read(*self.id(), conn)? {
            Ok(model.name)
        } else {
    ...
    /// Returns the topological graph of the procedure templates.
    fn graph(
        &self,
        conn: &mut PgConnection,
    ) -> Result<DiGraph<ProcedureTemplate>, diesel::result::Error> {
        let mut child_procedures =
            ParentProcedureTemplate::from_parent_procedure_template(self.id(), conn)?
                .into_iter()
                .filter_map(|procedure| {
                    let child_procedure = procedure.child_procedure_template(conn).ok()?;
                    let mut child_procedures =
                        ParentProcedureTemplate::from_parent_procedure_template(
                            &child_procedure.id,
                            conn,
                        )
                        .ok()?
                        .into_iter()
                        .filter_map(|p| p.child_procedure_template(conn).ok())
                        .collect::<Vec<_>>();
        ...
        let child_procedures: SortedVec<ProcedureTemplate> =
        ...
        let edges: SquareCSR2D<_> = DiEdgesBuilder::default()
            ...
            .edges(NextProcedureTemplate::from_parent(self.id(), conn)?.into_iter().filter_map(
                |next| {
        ...
        let graph: DiGraph<ProcedureTemplate> = GenericMonoplexMonopartiteGraphBuilder::default()
    ...
    /// Returns the IDs of the nodes in the procedure template, if any.
    ///
    /// # Arguments
    ///
    ...
    fn nodes(&self, conn: &mut PgConnection) -> Result<Vec<String>, diesel::result::Error> {
        ...
        let ordering = if graph.has_nodes() {
            ...
            let trackables = ProcedureTemplateAssetModel::from_procedure_template(self.id(), conn)?
                .into_iter()
    ...
    /// Returns the ID of the last node in the procedure template, if any.
    ///
    /// # Arguments
    ///
    /// * `conn` - A mutable reference to a `PgConnection` for database
    ...
    /// Returns the ID of the first node in the procedure template, if any.
    ///
    /// # Arguments
    ///
    /// * `conn` - A mutable reference to a `PgConnection` for database
    ...
    /// Returns the trackable nodes associated with the procedure template.
    ///
    /// # Arguments
    ///
    /// * `root_procedure_template` - The root procedure template to which the
    ///   trackables belong.
    /// * `conn` - A mutable reference to a `PgConnection` for database
    ///   operations.
    ///
    ...
    fn trackable_nodes(
        ...
        for procedure_trackable in
            ProcedureTemplateAssetModel::from_procedure_template(self.id(), conn)?
        {
            let root_trackable = root_procedure_template
                .procedure_template_trackable_equivalent(&procedure_trackable, None, conn)?
                .expect("Trackable should have an equivalent in the root procedure template");
    ...
    fn procedure_nodes_and_edges(
                ...
                .enumerate()
        {
            ...
            if child_procedure.last_node(conn)?.is_none() {
                ...
                dot.push_str(
                    child_procedure
                        .to_subgraph(&subgraph_prefix, root_procedure_template, conn)?
                        .as_str(),
                );
        ...
        for next in NextProcedureTemplate::from_parent(self.id(), conn)? {
    ...
    fn to_subgraph(
        ...
        if self.last_node(conn)?.is_none() {
            // If the procedure template has no trackables and no children, we display it as
            // a box
            return Ok(format!(
                "    P{} [label=\"{}\", shape=box, color={RED}];\n",
                self.id(),
        ...
        if ParentProcedureTemplate::from_parent_procedure_template(self.id(), conn)?.is_empty() {
            dot.push_str(self.trackable_nodes(root_procedure_template, conn)?.as_str());
        }
    ...
    /// Generates a DOT representation of the procedure template.
    ///
    /// # Arguments
    ///
    /// * `conn` - A mutable reference to a `PgConnection` for database
    ...
    fn to_dot(&self, conn: &mut PgConnection) -> Result<String, diesel::result::Error> {
        ...
        let root_procedure_template = ProcedureTemplate::read(*self.id(), conn)?
            .expect("Root procedure template should be present in the database");
        dot.push_str(self.procedure_nodes_and_edges("", &root_procedure_template, conn)?.as_str());
...
impl<T> ProcedureTemplateDot for T
where
