...
pub trait Constrainer: Default {
    ...
    fn register_table_constraint(
        &mut self,
        constraint: Box<dyn TableConstraint<Table = Self::Table>>,
    );
    ...
    fn table_constraints(&self) -> impl Iterator<Item = &dyn TableConstraint<Table = Self::Table>>;
    ...
    fn encounter_table(&self, table: &Self::Table) -> Result<(), Error> {
    ...
    /// its DB entities.
    fn validate_schema<DB: DatabaseLike<Table = Self::Table, Column = Self::Column>>(
        &self,
        database: &DB,
    ) -> Result<(), Error> {
        for table in database.tables() {
            self.encounter_table(table)?;
            for column in database.columns(table) {
