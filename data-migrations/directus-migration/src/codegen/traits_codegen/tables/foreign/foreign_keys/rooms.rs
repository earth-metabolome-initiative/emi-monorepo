...
pub struct RoomForeignKeys {
    ...
    pub user_updated:
        Option<std::rc::Rc<crate::codegen::structs_codegen::tables::directus_users::DirectusUser>>,
    pub building: Option<std::rc::Rc<crate::codegen::structs_codegen::tables::buildings::Building>>,
    pub address: Option<std::rc::Rc<crate::codegen::structs_codegen::tables::addresses::Address>>,
}
impl web_common_traits::prelude::HasForeignKeys
    for crate::codegen::structs_codegen::tables::rooms::Room
{
    type ForeignKeys = RoomForeignKeys;
        ...
        C: web_common_traits::crud::Connector<Row = Self::Row>,
    {
        ...
        connector.send(web_common_traits::crud::CrudPrimaryKeyOperation::Read(
            crate::codegen::tables::table_primary_keys::TablePrimaryKey::Building(self.building),
        ));
        connector.send(web_common_traits::crud::CrudPrimaryKeyOperation::Read(
            crate::codegen::tables::table_primary_keys::TablePrimaryKey::Address(self.address),
        ));
    }
    fn foreign_keys_loaded(&self, foreign_keys: &Self::ForeignKeys) -> bool {
        (foreign_keys.user_created.is_some() || self.user_created.is_none())
            ...
            && foreign_keys.building.is_some()
            && foreign_keys.address.is_some()
    }
    fn update(
        &self,
        ...
        match (row, crud) {
            (
                crate::codegen::tables::row::Row::Building(buildings),
                web_common_traits::crud::CRUD::Read
                | web_common_traits::crud::CRUD::Create
                | web_common_traits::crud::CRUD::Update,
            ) => {
                if buildings.id == self.building {
                    foreign_keys.building = Some(buildings);
                    updated = true;
                }
            }
            (
            ...
            (
                ...
                if addresses.id == self.address {
                    foreign_keys.address = Some(addresses);
                    updated = true;
                }
            }
            (
...
impl web_common_traits::prelude::ForeignKeys for RoomForeignKeys {}
