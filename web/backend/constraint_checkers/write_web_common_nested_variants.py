...
def write_web_common_nested_variants(nested_structs: List[StructMetadata]):
    ...
    if not (are_migrations_changed() or is_file_changed(__file__)):
        print("No change in migrations or file. Skipping writing frontend nested variants.")
        return

    # We open the file to write the nested structs
    document = open("../web_common/src/database/nested_variants.rs", "w", encoding="utf8")
