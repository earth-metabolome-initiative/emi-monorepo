//! Submodule implementing the `monoisotopic_mass` method for the `MolecularFormula`
//! struct
    ...
    pub fn monoisotopic_mass(&self) -> Option<f64> {
        match self {
            ...
            MolecularFormula::Solvation(solvation) => {
                match (solvation.solvate.monoisotopic_mass(), solvation.solvant.monoisotopic_mass()) {
                    (Some(solvate_monoisotopic_mass), Some(solvent_monoisotopic_mass)) => {
                        Some(solvate_monoisotopic_mass + solvent_monoisotopic_mass)
                    }
                    _ => None,
                }
            }
            MolecularFormula::Count(formula, count) => {
                formula.monoisotopic_mass().map(|monoisotopic_mass| monoisotopic_mass * f64::from(*count))
            }
