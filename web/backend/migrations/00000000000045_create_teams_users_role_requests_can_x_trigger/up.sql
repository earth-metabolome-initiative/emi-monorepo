-- The function `can_update_teams_users_role_requests` takes a user ID (INTEGER) and the primary keys
...
CREATE FUNCTION can_update_teams_users_role_requests(author_user_id INTEGER, table_id INTEGER, user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- We retrieve the value of the parent column from the row, as identified by the provided primary key(s).
    SELECT table_id, user_id, created_by, 1 INTO table_id, user_id, created_by, canary FROM teams_users_role_requests WHERE teams_users_role_requests.table_id = table_id AND teams_users_role_requests.user_id = user_id;
...
-- checks if the row was created by the user or if the user is found in either the teams_users_role_requests_users_roles table or
-- the teams_users_role_requests_teams_users table with an appropriate role id.
CREATE FUNCTION can_admin_teams_users_role_requests(author_user_id INTEGER, table_id INTEGER, user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    table_id INTEGER;
...
-- checks if the row was created by the user or if the user is found in either the teams_users_role_requests_users_roles table or
-- the teams_users_role_requests_teams_users table with an appropriate role id.
CREATE FUNCTION can_view_teams_users_role_requests(author_user_id INTEGER, table_id INTEGER, user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    table_id INTEGER;
