...
pub trait CommercialCentrifugeLotBuildable:
    ...
    + crate::codegen::structs_codegen::tables::insertables::CentrifugeModelBuildable
{
    ...
    /// `public.commercial_centrifuge_lots.product_model_id` column.
    ///
    /// # Arguments
...
impl CommercialCentrifugeLotBuildable for Option<i32> {
    fn product_model(
        self,
...
for InsertableCommercialCentrifugeLotBuilder<CentrifugeModel, CommercialProductLot> {
    ///Sets the value of the `public.commercial_centrifuge_lots.product_model_id` column.
    fn product_model(
        mut self,
        product_model_id: i32,
    ) -> Result<Self, web_common_traits::database::InsertError<Self::Attributes>> {
        let product_model_id = product_model_id
            ...
            .map_err(|err| {
                validation_errors::SingleFieldError::from(err)
                    .rename_field(
                        InsertableCommercialCentrifugeLotAttributes::ProductModelId,
                    )
            })?;
...
for InsertableCommercialCentrifugeLotBuilder<CentrifugeModel, CommercialProductLot> {
    ...
    {
        self.commercial_centrifuge_lots_id_fkey1 = <CentrifugeModel as crate::codegen::structs_codegen::tables::insertables::AssetModelBuildable>::most_concrete_table(
