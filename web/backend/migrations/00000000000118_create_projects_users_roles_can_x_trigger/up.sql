-- The function `can_update_projects_users_roles` takes a user ID (INTEGER) and the primary keys
...
-- checks if the row was created by the user or if the user is found in either the projects_users_roles_users_roles table or
-- the projects_users_roles_teams_users table with an appropriate role id.
CREATE FUNCTION can_update_projects_users_roles(author_user_id INTEGER, this_projects_users_roles_table_id INTEGER, this_projects_users_roles_user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- We retrieve the value of the parent column from the row, as identified by the provided primary key(s).
    SELECT table_id, user_id, created_by, 1 INTO this_table_id, this_user_id, this_created_by, canary FROM projects_users_roles WHERE projects_users_roles.table_id = this_projects_users_roles_table_id AND projects_users_roles.user_id = this_projects_users_roles_user_id;
...
-- The function `can_admin_projects_users_roles` takes a user ID (INTEGER) and the primary keys
-- and returns a BOOLEAN indicating whether the user can {operation} the row. Since this table's editability
-- may depend on the parent column, this function retrieves the value of the parent column from the row
-- and calls the parent column's can_admin function if the parent column is not NULL. Otherwise, the function
-- checks if the row was created by the user or if the user is found in either the projects_users_roles_users_roles table or
-- the projects_users_roles_teams_users table with an appropriate role id.
CREATE FUNCTION can_admin_projects_users_roles(author_user_id INTEGER, this_projects_users_roles_table_id INTEGER, this_projects_users_roles_user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    this_table_id INTEGER;
...
-- The function `can_view_projects_users_roles` takes a user ID (INTEGER) and the primary keys
-- and returns a BOOLEAN indicating whether the user can {operation} the row. Since this table's editability
-- may depend on the parent column, this function retrieves the value of the parent column from the row
-- and calls the parent column's can_view function if the parent column is not NULL. Otherwise, the function
-- checks if the row was created by the user or if the user is found in either the projects_users_roles_users_roles table or
-- the projects_users_roles_teams_users table with an appropriate role id.
CREATE FUNCTION can_view_projects_users_roles(author_user_id INTEGER, this_projects_users_roles_table_id INTEGER, this_projects_users_roles_user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    this_table_id INTEGER;
