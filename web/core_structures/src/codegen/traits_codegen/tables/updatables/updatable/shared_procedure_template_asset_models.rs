    ...
    >,
{
    ...
    fn can_update(
        ...
        if !self.parent_asset_model(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.parent_procedure_template(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.child(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.child_asset_model(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.child_procedure_template(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self
