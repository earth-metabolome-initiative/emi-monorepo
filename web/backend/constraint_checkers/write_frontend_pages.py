...
def write_frontend_pages(
    ...
    for builder in builders:
        ...
        if is_deny_listed(None, builder):
            continue
        ...
        function_component_name = flat_variant.human_readable_name().replace(" ", "_").lower() + "_page"
        ...
        for primary_key in flat_variant.get_primary_keys():
            document.write(
                f"    pub {primary_key.name}: {primary_key.data_type()},\n"
            )
        ...
        if len(primary_keys) == 1:
            ...
            document.write(
                f"impl {component_name}Prop {{\n"
            )

            for (_, foreign_key), child_struct in flat_variant.get_child_structs().items():
                ...
                # filter struct.
                assert child_struct.has_filter_variant(), (
