...
impl web_common_traits::database::TableField for MaterialAttribute {}
impl web_common_traits::database::HasTableType for MaterialAttribute {
    type Table = crate::codegen::tables::table_names::TableName;
}
impl core::fmt::Display for MaterialAttribute {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self {
            Self::Name => write!(f, "materials.name"),
            Self::Description => write!(f, "materials.description"),
            Self::Icon => write!(f, "materials.icon"),
            Self::ColorId => write!(f, "materials.color_id"),
            Self::Id => write!(f, "materials.id"),
        }
    }
}
#[derive(Debug)]
...
///
/// ```rust,ignore
/// use core_structures::Material;
/// use core_structures::tables::insertables::MaterialSettable;
        ...
        >,
{
        ...
        validation_errors::prelude::SingleFieldError: From<<N as TryInto<String>>::Error>,
    {
        let name = name.try_into().map_err(|err| {
            validation_errors::prelude::SingleFieldError::from(err)
                .rename_field(MaterialAttribute::Name)
        })?;
        self.name = Some(name);
        Ok(self)
    }
        ...
        validation_errors::prelude::SingleFieldError: From<<I as TryInto<String>>::Error>,
    {
        let icon = icon.try_into().map_err(|err| {
            validation_errors::prelude::SingleFieldError::from(err)
                .rename_field(MaterialAttribute::Icon)
        })?;
        self.icon = Some(icon);
        Ok(self)
    }
...
{
    type Error = web_common_traits::database::InsertError<MaterialAttribute>;
