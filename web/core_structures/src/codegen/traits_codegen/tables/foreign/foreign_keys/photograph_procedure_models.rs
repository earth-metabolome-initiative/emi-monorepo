...
for crate::codegen::structs_codegen::tables::photograph_procedure_models::PhotographProcedureModel {
    type ForeignKeys = PhotographProcedureModelForeignKeys;
        ...
        match (row, crud) {
            (
                ...
                if self.photographed_with == camera_models.id {
                    foreign_keys.photographed_with = Some(camera_models);
                    updated = true;
                }
            }
            (
...
impl web_common_traits::prelude::ForeignKeys for PhotographProcedureModelForeignKeys {}
