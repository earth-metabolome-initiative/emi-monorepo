...
pub struct BeadsModel {
    pub id: i32,
    pub diameter_millimeters: f32,
}
impl web_common_traits::prelude::TableName for BeadsModel {
    const TABLE_NAME: &'static str = "beads_models";
}
impl
...
impl diesel::Identifiable for BeadsModel {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
...
impl BeadsModel {
    ...
    pub fn from_parent_model(
        ...
        Self::table()
            .inner_join(
                physical_asset_models::table.on(beads_models::id.eq(physical_asset_models::id)),
            )
            .filter(physical_asset_models::parent_model.eq(parent_model))
            .order_by(beads_models::id.asc())
    ...
    pub fn from_name(
        ...
        use crate::codegen::diesel_codegen::tables::{
            asset_models::asset_models, beads_models::beads_models,
        };
        Self::table()
            .inner_join(asset_models::table.on(beads_models::id.eq(asset_models::id)))
...
impl AsRef<BeadsModel> for BeadsModel {
    fn as_ref(&self) -> &BeadsModel {
        self
    }
}
