...
pub struct TaggingProcedureTemplateForeignKeys {
...
for crate::codegen::structs_codegen::tables::tagging_procedure_templates::TaggingProcedureTemplate {
    ...
    fn foreign_keys_loaded(&self, foreign_keys: &Self::ForeignKeys) -> bool {
        foreign_keys.geolocated_with_model.is_some()
            ...
            && foreign_keys.tag_asset_model.is_some()
            && foreign_keys.tagged_asset_model.is_some()
    }
    fn update(
        &self,
        ...
        match (row, crud) {
            (
                ...
                if self.tag_asset_model == physical_asset_models.id {
                    foreign_keys.tag_asset_model = Some(physical_asset_models);
                    updated = true;
                }
                if self.tagged_asset_model == physical_asset_models.id {
                    foreign_keys.tagged_asset_model = Some(physical_asset_models);
                    updated = true;
                }
            }
            (
                ...
                if self.tag_asset_model == physical_asset_models.id {
                    foreign_keys.tag_asset_model = None;
                    updated = true;
                }
                if self.tagged_asset_model == physical_asset_models.id {
                    foreign_keys.tagged_asset_model = None;
                    updated = true;
                }
            }
            (
            ...
            (
                ...
                if self.procedure_template_tag_asset_model
                    == procedure_template_asset_models.id
                {
                    foreign_keys.procedure_template_tag_asset_model = Some(
                        procedure_template_asset_models.clone(),
                    );
                    updated = true;
                }
                if self.procedure_template_tagged_asset_model
                    == procedure_template_asset_models.id
                {
                    foreign_keys.procedure_template_tagged_asset_model = Some(
                        procedure_template_asset_models.clone(),
                    );
                    updated = true;
                }
...
impl web_common_traits::prelude::ForeignKeys for TaggingProcedureTemplateForeignKeys {}
