...
impl diesel::Identifiable for SamplingStepModel {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
}
impl SamplingStepModel {
    ...
    pub async fn from_created_by(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::sampling_step_models::sampling_step_models::dsl::created_by
                    .eq(created_by.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_updated_by(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::sampling_step_models::sampling_step_models::dsl::updated_by
                    .eq(updated_by.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_created_at(
        ...
        Self::table()
            .filter(sampling_step_models::created_at.eq(created_at))
            .order_by(sampling_step_models::id.asc())
            .load::<Self>(conn)
            .await
    }
    #[cfg(feature = "postgres")]
    pub async fn from_updated_at(
        ...
        use crate::codegen::diesel_codegen::tables::sampling_step_models::sampling_step_models;
        Self::table()
            .filter(sampling_step_models::updated_at.eq(updated_at))
            .order_by(sampling_step_models::id.asc())
            .load::<Self>(conn)
            .await
    }
