...
/// descendant tables.
pub trait BallMillMachineBuildable:
    crate::codegen::structs_codegen::tables::insertables::PhysicalAssetBuildable
{
    /// Sets the value of the `public.ball_mill_machines.model_id` column.
    ///
    /// # Arguments
    /// * `model_id`: The value to set for the
    ///   `public.ball_mill_machines.model_id` column.
    ///
...
impl BallMillMachineBuildable for Option<::rosetta_uuid::Uuid> {
    fn model(
        self,
        _model_id: i32,
    ) -> Result<Self, web_common_traits::database::InsertError<Self::Attributes>> {
        ...
        let model_id = model_id
            ...
            .map_err(|err| {
                validation_errors::SingleFieldError::from(err)
                    .rename_field(InsertableBallMillMachineAttributes::ModelId)
            })?;
...
for InsertableBallMillMachineBuilder<PhysicalAsset> {
    ...
    ///end
    ///subgraph v3 ["`ball_mill_machines`"]
    ...
    fn model(
        ...
        <Self as BallMillMachineBuildable>::model(self, model_id)
    }
    #[inline]
