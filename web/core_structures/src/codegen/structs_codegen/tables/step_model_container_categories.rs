...
impl diesel::Identifiable for StepModelContainerCategory {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
...
impl StepModelContainerCategory {
    ...
    #[cfg(feature = "postgres")]
    pub async fn from_procedure_model_container_category_id(
        conn: &mut diesel_async::AsyncPgConnection,
        procedure_model_container_category_id: &crate::codegen::structs_codegen::tables::procedure_model_container_categories::ProcedureModelContainerCategory,
    ) -> Result<Vec<Self>, diesel::result::Error> {
        use diesel::{ExpressionMethods, QueryDsl, associations::HasTable};
        use diesel_async::RunQueryDsl;
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::step_model_container_categories::step_model_container_categories::dsl::procedure_model_container_category_id
                    .eq(procedure_model_container_category_id.id),
            )
            .load::<Self>(conn)
            .await
    }
    ...
    pub async fn from_created_at(
        ...
        Self::table()
            .filter(step_model_container_categories::created_at.eq(created_at))
            .order_by(step_model_container_categories::id.asc())
            .load::<Self>(conn)
            .await
    }
    #[cfg(feature = "postgres")]
    pub async fn from_updated_at(
        ...
        use crate::codegen::diesel_codegen::tables::step_model_container_categories::step_model_container_categories;
        Self::table()
            .filter(step_model_container_categories::updated_at.eq(updated_at))
            .order_by(step_model_container_categories::id.asc())
            .load::<Self>(conn)
            .await
    }
