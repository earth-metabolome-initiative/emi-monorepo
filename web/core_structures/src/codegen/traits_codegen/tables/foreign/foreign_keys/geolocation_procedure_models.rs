...
for crate::codegen::structs_codegen::tables::geolocation_procedure_models::GeolocationProcedureModel {
    type ForeignKeys = GeolocationProcedureModelForeignKeys;
        ...
        match (row, crud) {
            (
                ...
                if self.geolocated_with == positioning_device_models.id {
                    foreign_keys.geolocated_with = Some(positioning_device_models);
                    updated = true;
                }
            }
            (
...
impl web_common_traits::prelude::ForeignKeys for GeolocationProcedureModelForeignKeys {}
