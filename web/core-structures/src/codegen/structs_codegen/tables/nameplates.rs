...
impl Nameplate {
    ...
    > {
        use diesel::{associations::HasTable, QueryDsl};
        use diesel_async::RunQueryDsl;
        crate :: codegen :: structs_codegen :: tables :: nameplate_categories :: NameplateCategory :: table () . find (& self . category_id) . first :: < crate :: codegen :: structs_codegen :: tables :: nameplate_categories :: NameplateCategory > (conn) . await
    }
    #[cfg(feature = "postgres")]
    pub async fn from_barcode(
        ...
        Self::table()
            .filter(diesel::ExpressionMethods::eq(
                crate::codegen::diesel_codegen::tables::nameplates::nameplates::barcode,
                barcode,
            ))
            .first::<Self>(conn)
            .await
