...
#[diesel(table_name = crate::database::schema::ranks)]
...
impl From<web_common::database::flat_variants::Rank> for crate::database::flat_variants::Rank {
    fn from(item: web_common::database::flat_variants::Rank) -> Self {
        Self {
            id: item.id,
            name: item.name,
...
impl From<crate::database::flat_variants::Rank> for web_common::database::flat_variants::Rank {
    fn from(item: crate::database::flat_variants::Rank) -> Self {
        Self {
            id: item.id,
            name: item.name,
            description: item.description,
...
impl Rank {
    ...
    pub fn all_viewable(
        ...
        use crate::database::schema::ranks;
        let query = ranks::dsl::ranks
            .select(Rank::as_select())
            .order_by(ranks::dsl::id);
        let mut query = query.into_boxed();
        if let Some(icon_id) = filter.and_then(|f| f.icon_id) {
            query = query.filter(ranks::dsl::icon_id.eq(icon_id));
        }
        if let Some(color_id) = filter.and_then(|f| f.color_id) {
            query = query.filter(ranks::dsl::color_id.eq(color_id));
        }
        query
    ...
    pub fn from_name(
        ...
        use crate::database::schema::ranks;
        let flat_variant = ranks::dsl::ranks
            .filter(ranks::dsl::name.eq(name))
            .first::<Self>(connection)?;
        Ok(flat_variant)
    }
    ...
    pub fn strict_word_similarity_search_viewable(
        ...
        use crate::database::schema::ranks;
        let mut query = ranks::dsl::ranks
            .select(Rank::as_select())
            .filter(
                crate::database::sql_function_bindings::concat_ranks_name_description(
                    ranks::dsl::name,
                    ranks::dsl::description,
                )
