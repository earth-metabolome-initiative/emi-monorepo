    ...
    I: AsRef<Image>,
{
    ...
    let (width, height) = image.shape().map_err(|_| validator::ValidationError::new("Invalid image data."))?;
    if (width as f32 / height as f32) > 1.5 || (height as f32 / width as f32) > 1.5 {
        return Err(validator::ValidationError::new("The image is not squarish."));
    }
    Ok(())
}