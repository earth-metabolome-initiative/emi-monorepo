...
pub struct ProcedureModelReagent {
    pub id: i32,
...
impl diesel::Identifiable for ProcedureModelReagent {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
}
impl ProcedureModelReagent {
    ...
    pub async fn from_procedure_model_id(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::procedure_model_reagents::procedure_model_reagents::dsl::procedure_model_id
    ...
    pub async fn from_reagent_id(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::procedure_model_reagents::procedure_model_reagents::dsl::reagent_id
                    .eq(reagent_id.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_created_by(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::procedure_model_reagents::procedure_model_reagents::dsl::created_by
                    .eq(created_by.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_updated_by(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::procedure_model_reagents::procedure_model_reagents::dsl::updated_by
                    .eq(updated_by.id),
            )
            .load::<Self>(conn)
            .await
    ...
    pub async fn from_created_at(
        ...
        use crate::codegen::diesel_codegen::tables::procedure_model_reagents::procedure_model_reagents;
        Self::table()
            .filter(procedure_model_reagents::created_at.eq(created_at))
            .order_by(procedure_model_reagents::id.asc())
            .load::<Self>(conn)
            .await
    }
    #[cfg(feature = "postgres")]
    pub async fn from_updated_at(
        ...
        use crate::codegen::diesel_codegen::tables::procedure_model_reagents::procedure_model_reagents;
        Self::table()
            .filter(procedure_model_reagents::updated_at.eq(updated_at))
            .order_by(procedure_model_reagents::id.asc())
            .load::<Self>(conn)
            .await
    }
