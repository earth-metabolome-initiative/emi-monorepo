...
use mermaid::{
    prelude::{
        ArrowShape, Color, Direction, Flowchart, FlowchartBuilder, FlowchartConfigurationBuilder,
        FlowchartEdgeBuilder, FlowchartNode, FlowchartNodeBuilder, FlowchartNodeShape, LineStyle,
        StyleClassBuilder, StyleProperty,
    },
    traits::{ConfigurationBuilder, DiagramBuilder, EdgeBuilder, NodeBuilder},
};
...
/// # Arguments
///
/// * `display_columns` - Whether to display the columns or only the tables.
...
pub(crate) fn columns_to_mermaid_illustration<C: AsRef<Column> + Ord + Eq + Hash>(
    ...
    let mut inferred_columns = columns
        .iter()
        .flat_map(|col| {
            let mut same_as = col.as_ref().all_ancestral_same_as_columns(conn).unwrap_or_default();
            same_as.extend(
                col.as_ref()
        ...
        .filter(|col| !columns.iter().any(|c| c.as_ref() == col))
        .collect::<Vec<_>>();
    inferred_columns.sort_unstable();
    inferred_columns.dedup();

    let mut flowchart = FlowchartBuilder::default()
        .configuration(FlowchartConfigurationBuilder::default().direction(Direction::BottomToTop))
        .unwrap();
    ...
    if display_columns {
        ...
        for column in &columns {
            ...
            node_builder = node_builder
                .style_class(Rc::clone(if column.as_ref() == column_of_interest {
                    &column_of_interest_class
                } else {
                    &directly_involved_column_class
                }))
                .unwrap();
            column_nodes.insert(column.as_ref(), flowchart.node(node_builder).unwrap());
        }
