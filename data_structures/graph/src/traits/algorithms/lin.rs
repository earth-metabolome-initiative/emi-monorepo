...
use crate::prelude::resnik::ResnikResult;
use functional_properties::prelude::ScalarSimilarity;

use crate::{
    prelude::information_content::InformationContentError,
    traits::{
        information_content::InformationContentResult, MonoplexMonopartiteGraph, Resnik
    },
};
...
impl <'graph, G: ?Sized + MonoplexMonopartiteGraph> From <ResnikResult<'graph, G>> for LinResult<'graph, G>{
...
impl <'graph, G: ?Sized + MonoplexMonopartiteGraph> AsRef<ResnikResult<'graph, G>> for LinResult<'graph, G> {
    fn as_ref(&self) -> &ResnikResult<'graph, G> {
        &self.0
    }
}

impl <'graph, G: ?Sized + MonoplexMonopartiteGraph> AsRef<InformationContentResult<'graph, G>> for LinResult<'graph, G> {
...
impl <G> ScalarSimilarity<G::NodeId, G::NodeId> for LinResult<'_, G> where G: MonoplexMonopartiteGraph, {
    type Similarity = f64;
    fn similarity(&self, left: &G::NodeId, right: &G::NodeId) -> Self::Similarity {
        ...
        2.0 * resnik_score / (f64::EPSILON + left_ic+right_ic)
    }
}
