...
for crate::codegen::structs_codegen::tables::commercial_pipette_tip_models::CommercialPipetteTipModel {
    ...
    fn foreign_keys_loaded(&self, foreign_keys: &Self::ForeignKeys) -> bool {
        foreign_keys.parent_model.is_some()
            && foreign_keys.commercial_pipette_tip_models_id_fkey.is_some()
            && foreign_keys.commercial_pipette_tip_models_id_fkey1.is_some()
    }
    fn update(
        &self,
        ...
        match (row, crud) {
            ...
            (
                ...
                if self.id == commercial_products.id {
                    foreign_keys.commercial_pipette_tip_models_id_fkey1 = None;
                    updated = true;
                }
            }
            (
                ...
                if self.parent_model == pipette_tip_models.id {
                    foreign_keys.parent_model = Some(pipette_tip_models);
                    updated = true;
                }
            ...
            (
                ...
                if self.id == pipette_tip_models.id {
                    foreign_keys.commercial_pipette_tip_models_id_fkey = None;
                    updated = true;
                }
            }
            (_, crud) => {
...
impl web_common_traits::prelude::ForeignKeys for CommercialPipetteTipModelForeignKeys {}
