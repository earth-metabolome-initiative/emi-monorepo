...
-- The function `can_update_nameplates_trigger` is a trigger function that checks whether the user can update the row.
CREATE FUNCTION can_update_nameplates_trigger()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        IF NOT can_update_nameplates(NEW.updated_by, NEW.id) THEN
            RAISE EXCEPTION 'unauthorized_update' USING DETAIL = 'Unauthorized to update this row of the `nameplates` table.';
        END IF;
    END IF;
...
-- We create a trigger that calls the `can_update_nameplates` function before each INSERT or UPDATE.
CREATE TRIGGER can_update_nameplates
BEFORE INSERT OR UPDATE ON nameplates
FOR EACH ROW
EXECUTE FUNCTION can_update_nameplates_trigger();
