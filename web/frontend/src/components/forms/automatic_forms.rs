...
impl FromOperation for SampledIndividualActions {
    fn from_operation<S: AsRef<str>>(operation: S, row: Vec<u8>) -> Self {
        match operation.as_ref() {
            "nameplate" => {
                SampledIndividualActions::SetNameplate(Some(bincode::deserialize(&row).unwrap()))
            }
            "project" => {
...
impl FormBuilder for SampledIndividualBuilder {
    ...
    fn update(
        ...
        dispatcher.apply(SampledIndividualActions::SetNameplate(
...
pub fn create_sampled_individual_form(props: &CreateSampledIndividualFormProp) -> Html {
    ...
    let set_nameplate = builder_dispatch.apply_callback(|nameplate: Option<NestedNameplate>| {
        SampledIndividualActions::SetNameplate(nameplate)
    });
    let set_project = builder_dispatch.apply_callback(|project: Option<NestedProject>| {
...
pub fn create_spectra_form(props: &CreateSpectraFormProp) -> Html {
    ...
    named_requests.push(ComponentMessage::get_named::<&str, SpectraCollection>(
        "spectra_collection",
        props.spectra_collection_id.into(),
    ));
    let set_notes =
