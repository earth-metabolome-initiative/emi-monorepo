...
use molecular_formula::{MolecularFormula, Token};
...
fn test_invalid_radicals() {
    assert_eq!(
        ...
        Err(molecular_formula::errors::Error::EmptyFormula)
    );

    assert_eq!(
        MolecularFormula::try_from("·+"),
        Err(molecular_formula::errors::Error::InvalidChargePosition)
    );
    ...
    assert_eq!(
        ...
        Err(molecular_formula::errors::Error::InvalidRepeatedToken(Token::Dot))
    );
}
