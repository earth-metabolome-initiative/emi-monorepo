-- The function `can_view_organism_taxa` takes a user ID (INTEGER) and the primary keys
...
-- checks if the row was created by the user or if the user is found in either the organism_taxa_users_roles table or
-- the organism_taxa_teams_users table with an appropriate role id.
CREATE FUNCTION can_view_organism_taxa(author_user_id INTEGER, this_organism_taxa_organism_id UUID, this_organism_taxa_taxon_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- We retrieve the value of the parent column from the row, as identified by the provided primary key(s).
    SELECT organism_id, created_by, 1 INTO this_organism_id, this_created_by, canary FROM organism_taxa WHERE organism_taxa.organism_id = this_organism_taxa_organism_id AND organism_taxa.taxon_id = this_organism_taxa_taxon_id;
...
-- The function `can_admin_organism_taxa` takes a user ID (INTEGER) and the primary keys
-- and returns a BOOLEAN indicating whether the user can {operation} the row. Since this table's editability
-- may depend on the parent column, this function retrieves the value of the parent column from the row
-- and calls the parent column's can_admin function if the parent column is not NULL. Otherwise, the function
-- checks if the row was created by the user or if the user is found in either the organism_taxa_users_roles table or
-- the organism_taxa_teams_users table with an appropriate role id.
CREATE FUNCTION can_admin_organism_taxa(author_user_id INTEGER, this_organism_taxa_organism_id UUID, this_organism_taxa_taxon_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    this_organism_id UUID;
