...
unsafe impl Send for SampleTaxonBuilder {}
unsafe impl Sync for SampleTaxonBuilder {}
impl Default for SampleTaxonBuilder {
    fn default() -> Self {
        Self {
...
pub(crate) enum SampleTaxonActions {
...
impl FromOperation for SampleTaxonActions {
    fn from_operation<S: AsRef<str>>(operation: S, row: Vec<u8>) -> Self {
        match operation.as_ref() {
            "sample" => SampleTaxonActions::SetSample(Some(bincode::deserialize(&row).unwrap())),
            "taxon" => SampleTaxonActions::SetTaxon(Some(bincode::deserialize(&row).unwrap())),
            operation_name => {
...
impl Reducer<SampleTaxonBuilder> for SampleTaxonActions {
    fn apply(self, mut state: std::rc::Rc<SampleTaxonBuilder>) -> std::rc::Rc<SampleTaxonBuilder> {
        let state_mut = Rc::make_mut(&mut state);
        match self {
            SampleTaxonActions::SetSample(sample) => 'sample: {
            ...
            SampleTaxonActions::SetTaxon(taxon) => 'taxon: {
...
impl FormBuilder for SampleTaxonBuilder {
    type Actions = SampleTaxonActions;

    type RichVariant = NestedSampleTaxon;
    ...
    fn update(
        ...
        dispatcher.apply(SampleTaxonActions::SetSample(
            Some(richest_variant.sample).map(Rc::from),
        ));
        dispatcher.apply(SampleTaxonActions::SetTaxon(
...
impl From<SampleTaxonBuilder> for NewSampleTaxon {
    fn from(builder: SampleTaxonBuilder) -> Self {
        Self {
            sample_id: builder
                .sample
                .as_ref()
...
impl FormBuildable for NewSampleTaxon {
    type Builder = SampleTaxonBuilder;
    fn title() -> &'static str {
        "Sample taxon"
    }
...
pub struct CreateSampleTaxonFormProp {
    #[prop_or_default]
...
#[function_component(CreateSampleTaxonForm)]
pub fn create_sample_taxon_form(props: &CreateSampleTaxonFormProp) -> Html {
    let mut named_requests: Vec<ComponentMessage> = Vec::new();
    let (builder_store, builder_dispatch) = yewdux::use_store::<SampleTaxonBuilder>();
