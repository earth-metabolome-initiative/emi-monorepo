...
CREATE FUNCTION can_update_users_users_role_invitations(author_user_id INTEGER, this_users_users_role_invitations_table_id INTEGER, this_users_users_role_invitations_user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- We retrieve the value of the parent column from the row, as identified by the provided primary key(s).
    SELECT table_id, user_id, created_by, 1 INTO this_table_id, this_user_id, this_created_by, canary FROM users_users_role_invitations WHERE users_users_role_invitations.table_id = this_users_users_role_invitations_table_id AND users_users_role_invitations.user_id = this_users_users_role_invitations_user_id;
...
-- checks if the row was created by the user or if the user is found in either the users_users_role_invitations_users_roles table or
-- the users_users_role_invitations_teams_users table with an appropriate role id.
CREATE FUNCTION can_admin_users_users_role_invitations(author_user_id INTEGER, this_users_users_role_invitations_table_id INTEGER, this_users_users_role_invitations_user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    this_table_id INTEGER;
...
-- checks if the row was created by the user or if the user is found in either the users_users_role_invitations_users_roles table or
-- the users_users_role_invitations_teams_users table with an appropriate role id.
CREATE FUNCTION can_view_users_users_role_invitations(author_user_id INTEGER, this_users_users_role_invitations_table_id INTEGER, this_users_users_role_invitations_user_id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    this_table_id INTEGER;
