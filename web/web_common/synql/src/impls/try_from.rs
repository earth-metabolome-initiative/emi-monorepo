//! Submodule providing implementations of the `TryFrom` trait to attempt
//! conversion of various structs from the [`pg_diesel`](crate) into the
//! corresponding `syn` AST nodes.
...
pub fn to_struct_or_enum(&self, conn: &mut PgConnection) -> Result<TokenStream, WebCodeGenError> {
