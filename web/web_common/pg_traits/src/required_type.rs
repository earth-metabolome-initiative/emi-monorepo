...
#[derive(Clone)]
/// Struct defining the type required by some type found in the postgres database schema.
...
impl RequiredType {
    ...
    /// Returns the diesel type defined within the crate compatible with the given postgres type.
    pub fn diesel_type(&self) -> &syn::Type {
        &self.diesel_type
    }

    /// Returns the rust type defined within the crate compatible with the given postgres type.
    pub fn rust_type(&self) -> &syn::Type {
        &self.rust_type
    }

    /// Returns a reference over the postgres types which are compatible with the diesel and rust types defined within the crate.
    pub fn postgres_types(&self) -> &[&'static str] {
        &self.postgres_types
    }

    /// Returns whether the current `RequiredType` is compatible with the given postgres type.
    ///
    /// # Arguments
