...
pub trait CommercialPositioningDeviceLotBuildable:
    ...
    + crate::codegen::structs_codegen::tables::insertables::PositioningDeviceModelBuildable
{
    ...
    /// `public.commercial_positioning_device_lots.product_model_id` column.
    ///
    /// # Arguments
...
impl CommercialPositioningDeviceLotBuildable for Option<i32> {
    fn product_model(
        self,
...
for InsertableCommercialPositioningDeviceLotBuilder<
    ...
    PositioningDeviceModel,
> {
    ///Sets the value of the `public.commercial_positioning_device_lots.product_model_id` column.
    fn product_model(
        mut self,
        product_model_id: i32,
    ) -> Result<Self, web_common_traits::database::InsertError<Self::Attributes>> {
        let product_model_id = product_model_id
            ...
            .map_err(|err| {
                validation_errors::SingleFieldError::from(err)
                    .rename_field(
                        InsertableCommercialPositioningDeviceLotAttributes::ProductModelId,
                    )
            })?;
...
> {
    ...
    {
        self.commercial_positioning_device_lots_id_fkey = <CommercialProductLot as crate::codegen::structs_codegen::tables::insertables::AssetModelBuildable>::most_concrete_table(
