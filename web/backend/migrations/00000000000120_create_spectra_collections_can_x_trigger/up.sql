...
CREATE OR REPLACE FUNCTION can_update_spectra_collections(author_user_id INTEGER, id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
...
-- The function `can_update_spectra_collections_trigger` is a trigger function that checks whether the user can edit the row.
CREATE OR REPLACE FUNCTION can_update_spectra_collections_trigger()
RETURNS TRIGGER AS $$
BEGIN
...
-- checks if the row was created by the user or if the user is found in either the spectra_collections_users_roles table or
-- the spectra_collections_teams_users table with an appropriate role id.
CREATE OR REPLACE FUNCTION can_admin_spectra_collections(author_user_id INTEGER, id INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    canary INTEGER; -- Value used to check whether the row we are queering for actually exists, so to distinguish when the parent column is NULL and when the row is missing.
    sample_id UUID;
