...
pub struct BatchForeignKeys {
    pub user_created:
        Option<std::rc::Rc<crate::codegen::structs_codegen::tables::directus_users::DirectusUser>>,
    pub user_updated:
        Option<std::rc::Rc<crate::codegen::structs_codegen::tables::directus_users::DirectusUser>>,
    pub batch_type:
        Option<std::rc::Rc<crate::codegen::structs_codegen::tables::batch_types::BatchType>>,
}
impl web_common_traits::prelude::HasForeignKeys
    for crate::codegen::structs_codegen::tables::batches::Batch
{
    type ForeignKeys = BatchForeignKeys;
        ...
        C: web_common_traits::crud::Connector<Row = Self::Row>,
    {
        ...
        if let Some(batch_type) = self.batch_type {
            connector.send(web_common_traits::crud::CrudPrimaryKeyOperation::Read(
                crate::codegen::tables::table_primary_keys::TablePrimaryKey::BatchType(batch_type),
            ));
        }
    }
    fn foreign_keys_loaded(&self, foreign_keys: &Self::ForeignKeys) -> bool {
        (foreign_keys.user_created.is_some() || self.user_created.is_none())
            ...
            && (foreign_keys.batch_type.is_some() || self.batch_type.is_none())
    }
    fn update(
        &self,
        ...
        match (row, crud) {
            ...
            (
                crate::codegen::tables::row::Row::BatchType(batch_types),
                web_common_traits::crud::CRUD::Read
                | web_common_traits::crud::CRUD::Create
                | web_common_traits::crud::CRUD::Update,
            ) => {
                if let Some(batch_type) = self.batch_type {
                    if batch_types.id == batch_type {
                        foreign_keys.batch_type = Some(batch_types);
                        updated = true;
                    }
                }
            }
...
impl web_common_traits::prelude::ForeignKeys for BatchForeignKeys {}
