...
CREATE TABLE IF NOT EXISTS container_compatibility_rules (
    ...
    PRIMARY KEY (container_model_id, contained_asset_model),
    CHECK (
        must_be_distinct_i32(container_model_id, contained_asset_model)
    )
);
...
CREATE TABLE IF NOT EXISTS volumetric_containers (
    ...
    -- We ensure that the parent table's container_model_id is indeed a volumetric_container_model_id.
