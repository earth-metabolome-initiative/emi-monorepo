...
impl SampleState {
    ...
    pub async fn from_icon_id(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::sample_states::sample_states::dsl::icon_id
                    .eq(icon_id.id),
            )
            .first::<Self>(conn)
            .await
    ...
    pub async fn from_color_id(
        ...
        Self::table()
            .filter(
                crate::codegen::diesel_codegen::tables::sample_states::sample_states::dsl::color_id
                    .eq(color_id.id),
            )
            .first::<Self>(conn)
            .await
