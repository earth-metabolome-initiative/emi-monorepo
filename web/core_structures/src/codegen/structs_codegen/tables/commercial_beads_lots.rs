...
pub struct CommercialBeadsLot {
    pub id: i32,
    pub product_model_id: i32,
}
impl web_common_traits::prelude::TableName for CommercialBeadsLot {
    const TABLE_NAME: &'static str = "commercial_beads_lots";
}
impl
    web_common_traits::prelude::ExtensionTable<
        crate::codegen::structs_codegen::tables::asset_models::AssetModel,
    > for CommercialBeadsLot
where
...
impl diesel::Identifiable for CommercialBeadsLot {
    type Id = i32;
    fn id(self) -> Self::Id {
        self.id
    }
}
impl CommercialBeadsLot {
    ...
    pub fn from_product_model_id(
        ...
        use crate::codegen::diesel_codegen::tables::commercial_beads_lots::commercial_beads_lots;
        Self::table()
            .filter(commercial_beads_lots::product_model_id.eq(product_model_id))
            .order_by(commercial_beads_lots::id.asc())
            .load::<Self>(conn)
    }
    #[cfg(feature = "postgres")]
    pub fn from_id_and_product_model_id(
        ...
        Self::table()
            .filter(
                commercial_beads_lots::id
                    .eq(id)
                    .and(commercial_beads_lots::product_model_id.eq(product_model_id)),
            )
            .order_by(commercial_beads_lots::id.asc())
            .load::<Self>(conn)
    }
    ...
    pub fn from_lot_and_product_model_id(
        ...
        use crate::codegen::diesel_codegen::tables::{
            commercial_beads_lots::commercial_beads_lots,
            commercial_product_lots::commercial_product_lots,
        };
        Self::table()
            .inner_join(
                commercial_product_lots::table
                    .on(commercial_beads_lots::id.eq(commercial_product_lots::id)),
            )
            .filter(
        ...
        Self::table()
            .inner_join(
                physical_asset_models::table
                    .on(commercial_beads_lots::id.eq(physical_asset_models::id)),
            )
    ...
    pub fn from_parent_model_id_and_id(
        ...
        Self::table()
            .inner_join(asset_models::table.on(commercial_beads_lots::id.eq(asset_models::id)))
...
impl AsRef<CommercialBeadsLot> for CommercialBeadsLot {
    fn as_ref(&self) -> &CommercialBeadsLot {
        self
    }
}
