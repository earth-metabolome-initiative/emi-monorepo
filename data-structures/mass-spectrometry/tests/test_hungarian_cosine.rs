...
pub fn test_hungarian_cosine() {
    ...
    // let (similarity, number_of_shared_peaks) = cosine.similarity(&cocaine,
    // &glucose); assert!(similarity <= 0.01, "Similarity: {}", similarity);
    // assert_eq!(number_of_shared_peaks, 0);

    let cosine2 = ExactCosine::new(0.0, 1.0, 5.0);
    // let (similarity, number_of_shared_peaks) = cosine2.similarity(&cocaine,
    // &glucose); assert!(
    ...
    // let (self_cocaine_similarity, number_of_shared_peaks) =
    // cosine.similarity(&cocaine, &cocaine);
    // assert_eq!(self_cocaine_similarity, 1.0);
    // assert_eq!(number_of_shared_peaks, cocaine.len() as u16);
    // let (self_glucose_similarity, number_of_shared_peaks) =
    // cosine.similarity(&glucose, &glucose);
    ...
    // let (similarity, number_of_shared_peaks) = cosine.similarity(&aspirin,
    // &salicin); assert!(similarity <= 0.01);
    // assert_eq!(number_of_shared_peaks, 0);
}
