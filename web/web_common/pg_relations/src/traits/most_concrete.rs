//! Submodule defining the `MostConcrete` trait which provides method relative to
//! dispatching the most-concrete table in an inheritance hierarchy.
...
/// i.e. a column indicating the most concrete table in an inheritance hierarchy.
pub trait MostConcrete: Extension {
    /// Returns the column which indicates the most concrete table in an inheritance hierarchy, if any.
	///
	/// # Arguments
	///
	/// * `recursive` - Whether to search ancestor tables for a `most_concrete_table` column.
