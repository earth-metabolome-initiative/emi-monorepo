...
impl From<web_common::database::nested_variants::NestedSampleState> for NestedSampleState {
    fn from(item: web_common::database::nested_variants::NestedSampleState) -> Self {
        Self {
            inner: SampleState::from(item.inner.as_ref().clone()),
            icon: FontAwesomeIcon::from(item.icon.as_ref().clone()),
            color: Color::from(item.color.as_ref().clone()),
        }
    }
}
impl From<NestedSampleState> for web_common::database::nested_variants::NestedSampleState {
    fn from(item: NestedSampleState) -> Self {
        Self {
            inner: Rc::from(web_common::database::SampleState::from(item.inner)),
