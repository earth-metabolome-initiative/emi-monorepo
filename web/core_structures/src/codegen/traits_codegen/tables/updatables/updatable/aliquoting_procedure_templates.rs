    ...
    crate::ProcedureTemplateAssetModel: web_common_traits::database::Updatable<C, UserId = i32>,
{
    ...
    fn can_update(
        ...
        if !self.procedure_template_aliquoted_into_model(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
        if !self.procedure_template_aliquoted_with_model(conn)?.can_update(user_id, conn)? {
            return Ok(false);
        }
